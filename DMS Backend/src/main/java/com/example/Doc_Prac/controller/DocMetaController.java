...
public class DocMetaController {
    ...
    @PutMapping("/file/add")
    public ResponseEntity<?> addImage(@RequestParam("ticketID") String ticketID, @RequestParam("id") int id, @RequestPart("file") MultipartFile multipartFile) throws IOException {
        ...
        if(extension[1].equalsIgnoreCase("jpg") || extension[1].equalsIgnoreCase("jpeg") || extension[1].equalsIgnoreCase("pdf") || extension[1].equalsIgnoreCase("zip")){
            return new ResponseEntity<>(docMetaService.addFile(ticketID,multipartFile),HttpStatus.OK);
        }
        else{
